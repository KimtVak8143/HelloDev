...
async function startTask(bugId, developerName) {
  ...
  await notion.pages.update({ page_id: task.id, properties: { Status: { select: { name: "In Progress" } } } });
  ...
  await notion.pages.update({ page_id: task.id, properties: { "Log Sheet": { relation: [{ id: log.id }] } } });
...
async function completeTask(bugId, logId, stats) {
  ...
  await notion.pages.update({ page_id: task.id, properties: { Status: { select: { name: "Done" } } } });
  logger.success(`Task "${taskName}" marked Done`);

  logger.notion("Sealing Log Sheet", { logId, hours: stats.hours.toFixed(2), commits: stats.commits });
  await notion.pages.update({
    ...
    properties: {
      ...
      "Commit Messages":  { rich_text: [{ text: { content: stats.messages.join(" | ") || "No commits" } }] },
